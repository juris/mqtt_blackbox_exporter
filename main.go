...
func startProbe(probeConfig *probeConfig) {
	...
	tlsconfig, _ := newTLSConfig(probeConfig)
	...
	subscriberOptions := mqtt.NewClientOptions().SetClientID(probeConfig.ClientPrefix).SetUsername(probeConfig.Username).SetPassword(probeConfig.Password).SetTLSConfig(&tlsconfig).AddBroker(probeConfig.BrokerIn)
